...
// Fifty rather than forty since the retune: an ordinary player's round used
// to be over in 58 seconds, which is a punishment rather than a contest. The
// ceiling is still here because the mode has to open tense — `npm run
// timeattack` is what says whether it still does.
check("and the clock is the tense kind, not a comfortable one", opening !== null && opening <= 55, `${opening}s`);
