...
if (import.meta.env.DEV) {
  (window as unknown as { __shiftle: unknown }).__shiftle = {
    ...
    botMove: () => {
      ...
      // V2, not the default V1. `chooseMove` defaults to a policy that never
      // fires the core, which was harmless until the core became a way *out*
      // of a stuck board: a round no longer ends while a full core is sitting
      // there, so a bot that will not fire one simply stops — measured, it
      // stalls on a ready core in 12 rounds out of 12 and its median depth
      // falls from 13 to 8. That made every browser check downstream of "play
      // until depth N" a coin flip. The bot here stands in for a player, and a
      // player fires the core.
      const move = chooseMove(current, BOT_POLICY_V2);
